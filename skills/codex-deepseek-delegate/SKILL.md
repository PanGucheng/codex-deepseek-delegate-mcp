...
Omit `maxTurns` for normal work; the MCP will set `maxTurns` to 100 to avoid lower worker defaults. Pass a smaller value only when intentionally limiting a smoke test or budget.
